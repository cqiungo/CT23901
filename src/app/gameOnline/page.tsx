...
function ScoreText({ score, room, player }:any) {
  ...
  const [position,setPosition]=useState<[number,number,number]>([0,10,0])
  useEffect(() => {
    if (!room) return;
...
export default function GameLayoutOnline() {
  ...
  const [score,setScores] =useState<Scores>(room?.scores ?? {})
  ...
  useEffect(() => {
    ...
    const isHost = player === room.host;

    if (isHost) {
      // Player 1 góc nhìn phía trước
      camera.position.set(0, 12, 20);
    } else {
      // Player 2 nhìn từ phía sau
      camera.position.set(0, 12, -20);
    }
...
{room && (
      <div className="absolute top-20 left-5 bg-white/90 p-4 rounded-lg shadow-lg z-20 w-64">
        <h2 className="font-bold text-lg mb-2">Thông tin phòng</h2>

        <p><strong>Tên phòng:</strong> {room.name}</p>
        <p><strong>ID:</strong> {room.id}</p>
        <p><strong>Chủ phòng:</strong> {room.host}</p>

        <div className="mt-2">
          <h3 className="font-semibold">Người chơi:</h3>

          {room.players?.map((p) => (
            <div key={p} className="flex justify-between px-2">
              <span>{p}</span>
            </div>
          ))}
        </div>
  ...
  <>
    ...
    <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-20">
      ...
      > 
        ...
        <ScoreText score={score} room={room} player={player} />
